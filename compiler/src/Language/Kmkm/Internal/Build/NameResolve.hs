...
dataRepresentation :: (MonadThrow m, Traversable f, Copointed f, S.HasLocation f) => Map S.Identifier Affiliation -> S.ModuleName -> f (DataRepresentation 'S.NameUnresolved et ev f) -> m (f (DataRepresentation 'S.NameResolved et ev f))
