...
module Language.Kmkm.Internal.Syntax.Core.NameUnresolved.Untyped.Curried.LambdaUnlifted
...
import           Language.Kmkm.Internal.Syntax.Core.Common                 (Constraint,
                                                                            Literal (Fraction, Integer, String, base, exponent, fractionDigits, significand, value),
                                                                            ModuleName, Pretty (pretty), Pretty1,
                                                                            prettyList)
import           Language.Kmkm.Internal.Syntax.Core.NameUnresolved         (BindIdentifier, ReferenceIdentifier)
import           Language.Kmkm.Internal.Syntax.Core.NameUnresolved.Curried (Type (ForAllType, FunctionType, ProcedureType, TypeApplication, TypeVariable))
...
instance (FunctorB et, FunctorB ev) => FunctorB (Module et ev) where
  bmap f (Module n ms ds) = Module (F.map f n) (fmap (F.map f) <$> F.map f ms) (fmap (F.map $ fmap (bmap f) . f) <$> F.map f ds)
...
data DataRepresentation et ev f
  = ForAllData (F f BindIdentifier) (F f (DataRepresentation et ev f))
  | ValueConstructorsData (F f [F f (ValueConstructor et ev f)])
...
instance (FunctorB et, FunctorB ev) => FunctorB (DataRepresentation et ev) where
  bmap f (ForAllData i r)           = ForAllData (F.map f i) $ bmap f <$> F.map f r
  bmap f (ValueConstructorsData cs) = ValueConstructorsData $ fmap (F.map $ fmap (bmap f) . f) <$> F.map f cs

instance Pretty1 f => Pretty (DataRepresentation et ev f) where
  pretty (ForAllData i rs)          = prettyList "for-all" [pretty i, pretty rs]
...
instance FunctorB (ValueConstructor et ev) where
  bmap f (ValueConstructor i fs) = ValueConstructor (F.map f i) (fmap (F.map $ fmap (bmap f) . f) <$> F.map f fs)
...
data Value et ev f
  ...
  | Literal (F f Literal)
  | Function (F f BindIdentifier) (F f (Type f)) (F f (Value et ev f))
  | Application (F f (Value et ev f)) (F f (Value et ev f))
  | Procedure (F f (NonEmpty (F f (ProcedureStep et ev f))))
  | TypeAnnotation (F f (Value et ev f)) (F f (Type f))
...
instance (FunctorB et, FunctorB ev) => FunctorB (Value et ev) where
  ...
  bmap f (Function i t v)     = Function (F.map f i) (bmap f <$> F.map f t) (bmap f <$> F.map f v)
  bmap f (Application v1 v2)  = Application (bmap f <$> F.map f v1) (bmap f <$> F.map f v2)
  bmap f (Procedure ps)       = Procedure $ fmap (F.map $ fmap (bmap f) . f) <$> F.map f ps
  bmap f (TypeAnnotation v t) = TypeAnnotation (bmap f <$> F.map f v) (bmap f <$> F.map f t)
  bmap f (Let ds v)           = Let (fmap (F.map $ fmap (bmap f) . f) <$> F.map f ds) $ bmap f <$> F.map f v
...
instance Constraint Pretty Pretty1 et ev f => Pretty (Value et ev f) where
  ...
  pretty (Function n t v)     = prettyList "function" [pretty n, pretty t, pretty v]
