...
module Language.Kmkm.Internal.Syntax.Core.NameResolved.Uncurried
  ( -- * Types
    Type (..)
  ) where
...
data Type f
  ...
  | FunctionType (F f [F f (Type f)]) (F f (Type f))
...
instance FunctorB Type where
  ...
  bmap f (FunctionType t1 t2)    = FunctionType (fmap (fmap (bmap f) . F.map f) <$> F.map f t1) (bmap f <$> F.map f t2)
