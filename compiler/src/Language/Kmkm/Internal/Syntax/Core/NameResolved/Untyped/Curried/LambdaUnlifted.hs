...
module Language.Kmkm.Internal.Syntax.Core.NameResolved.Untyped.Curried.LambdaUnlifted
...
import           Language.Kmkm.Internal.Syntax.Core.Common               (Constraint, EmbeddedCType (EmbeddedCType),
                                                                          ...
                                                                          isEmbeddedCType, isEmbeddedCValue, prettyList)
import           Language.Kmkm.Internal.Syntax.Core.NameResolved         (BindIdentifier, ReferenceIdentifier)
import           Language.Kmkm.Internal.Syntax.Core.NameResolved.Curried (Type (ForAllType, FunctionType, ProcedureType, TypeApplication, TypeVariable))
...
instance (FunctorB et, FunctorB ev) => FunctorB (DataRepresentation et ev) where
  ...
  bmap f (ValueConstructorsData cs) = ValueConstructorsData $ fmap (fmap (bmap f) . F.map f) <$> F.map f cs
