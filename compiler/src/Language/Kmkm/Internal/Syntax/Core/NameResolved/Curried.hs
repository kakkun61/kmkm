...
module Language.Kmkm.Internal.Syntax.Core.NameResolved.Curried
  ( -- * Types
    Type (..)
  ) where
...
instance Pointed f => IsString (Type f) where
  fromString = TypeVariable . point . fromString

instance (E.Item ReferenceIdentifier ~ Text, Pointed f, Copointed f) => IsList (Type f) where
