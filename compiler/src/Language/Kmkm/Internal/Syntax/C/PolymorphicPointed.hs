...
module Language.Kmkm.Internal.Syntax.C.PolymorphicPointed
...
data Definition et ev f
  ...
  | ValueBindV (F f BindIdentifier) (F f [F f BindIdentifier]) (F f (Value et ev f))
  | ValueBindN (F f BindIdentifier) (F f [F f BindIdentifier]) (F f [F f (F f BindIdentifier, F f (Type f))]) (F f (Value et ev f))
...
instance (FunctorB et, FunctorB ev) => FunctorB (Definition et ev) where
  ...
  bmap f (ValueBindV i ts v)          = ValueBindV (F.map f i) (fmap (F.map f) <$> F.map f ts) (bmap f <$> F.map f v)
  bmap f (ValueBindN i ts is v)          = ValueBindN (F.map f i) (fmap (F.map f) <$> F.map f ts) (fmap (fmap (bimap (F.map f) (fmap (bmap f) . F.map f)) <$> F.map f) <$> F.map f is) (bmap f <$> F.map f v)
...
instance Constraint Pretty Pretty1 et ev f => Pretty (Definition et ev f) where
  ...
  pretty (ValueBindV i ts v)      = prettyList "bind-value" [pretty i, pretty ts, pretty v]
  pretty (ValueBindN i ts is v)   = prettyList "bind-value" [pretty i, pretty ts, pretty is, pretty v]
...
data Value et ev f =
  TypedValue (F f (Value' et ev f)) (F f (Type f)) PolymorphicPoint
  deriving Generic
...
instance (FunctorB et, FunctorB ev) => FunctorB (Value et ev) where
  bmap f (TypedValue v t p) = TypedValue (bmap f <$> F.map f v) (bmap f <$> F.map f t) p

instance Constraint Pretty Pretty1 et ev f => Pretty (Value et ev f) where
  pretty (TypedValue v t _) = prettyList "type" [pretty v, pretty t]
...
data ParameterPolymorphic = Monoparam | Polyparam deriving (Show, Read, Eq, Ord, Generic)

data PolymorphicPoint = PolymorphicPoint ParameterPolymorphic [ParameterPolymorphic] deriving (Show, Read, Eq, Ord, Generic)
