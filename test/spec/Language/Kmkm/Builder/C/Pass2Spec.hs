    ...
    describe "member" $ do
      describe "v" $ do
        it "foo :: int ⇒ int foo" $ do
          member def (S.ValueBind (S.ValueBindV "foo" $ V.TypedTerm (V.Literal $ V.Integer 10 10) $ T.Variable "int") [])
            `shouldBe`
        ...
        it "foo :: int → int ⇒ int (*foo)(int)" $ do
          member def (S.ValueBind (S.ValueBindV "foo" $ V.TypedTerm (V.Variable "bar") $ T.Function $ T.FunctionN [T.Variable "int"] $ T.Variable "int") [])
            `shouldBe`
      ...
      describe "0" $ do
        it "foo :: () → int ⇒ int foo()" $ do
          member def (S.ValueBind (S.ValueBindN "foo" [] $ V.TypedTerm (V.Literal $ V.Integer 10 10) $ T.Variable "int") [])
            `shouldBe`
              [C.Definition (C.StatementDefinition ([], C.Int) [] "foo" [C.Function []] [C.BlockStatement $ C.Return $ C.Literal $ C.Integer 10 C.IntDecimal])]

        it "foo :: () → (int → int) ⇒ int (*foo())(int)" $ do
          member def (S.ValueBind (S.ValueBindN "foo" [] $ V.TypedTerm (V.Variable "bar") $ T.Function $ T.FunctionN [T.Variable "int"] $ T.Variable "int") [])
            `shouldBe`
      ...
      describe "1" $ do
        it "foo :: int → int ⇒ int foo(int a)" $ do
          member def (S.ValueBind (S.ValueBindN "foo" [("a", T.Variable "int")] $ V.TypedTerm (V.Literal $ V.Integer 10 10) $ T.Variable "int") [])
            `shouldBe`
        ...
        it "foo :: (int → int) → int ⇒ int foo(int (*a)(int))" $ do
          member def (S.ValueBind (S.ValueBindN "foo" [("a", T.Function $ T.FunctionN [T.Variable "int"] $ T.Variable "int")] $ V.TypedTerm (V.Variable "bar") $ T.Variable "int") [])
            `shouldBe`
