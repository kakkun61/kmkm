    ...
    describe "definition" $ do
      describe "v" $ do
        it "foo :: int ⇒ int foo" $ do
          definition def "spec" True (S.ValueBind (S.ValueBindV "foo" $ S.TypedTerm (S.Literal $ S.Integer 10 10) $ TypeVariable "int"))
            `shouldBe`
        ...
        it "foo :: int → int ⇒ int (*foo)(int)" $ do
          definition def "spec" True (S.ValueBind (S.ValueBindV "foo" $ S.TypedTerm (S.Variable $ QualifiedIdentifier (Just "spec") "bar") $ FunctionType $ FunctionTypeN [TypeVariable "int"] $ TypeVariable "int"))
            `shouldBe`
      ...
      describe "0" $ do
        it "foo :: () → int ⇒ int foo()" $ do
          definition def "spec" True (S.ValueBind (S.ValueBindN "foo" [] $ S.TypedTerm (S.Literal $ S.Integer 10 10) $ TypeVariable "int"))
            `shouldBe`
        ...
        it "foo :: () → (int → int) ⇒ int (*foo())(int)" $ do
          definition def "spec" True (S.ValueBind (S.ValueBindN "foo" [] $ S.TypedTerm (S.Variable $ QualifiedIdentifier (Just "spec") "bar") $ FunctionType $ FunctionTypeN [TypeVariable "int"] $ TypeVariable "int"))
            `shouldBe`
      ...
      describe "1" $ do
        it "foo :: int → int ⇒ int foo(int a)" $ do
          definition def "spec" True (S.ValueBind (S.ValueBindN "foo" [("a", TypeVariable "int")] $ S.TypedTerm (S.Literal $ S.Integer 10 10) $ TypeVariable "int"))
            `shouldBe`
        ...
        it "foo :: (int → int) → int ⇒ int foo(int (*a)(int))" $ do
          definition def "spec" True (S.ValueBind (S.ValueBindN "foo" [("a", FunctionType $ FunctionTypeN [TypeVariable "int"] $ TypeVariable "int")] $ S.TypedTerm (S.Variable $ QualifiedIdentifier (Just "spec") "bar") $ TypeVariable "int"))
            `shouldBe`
